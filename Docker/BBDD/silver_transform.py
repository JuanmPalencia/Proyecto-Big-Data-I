...
def safe_float(v):
    """Convierte a float; devuelve None si es NaN, inf o None."""
    if v is None:
        return None
    try:
        import math
        f = float(v)
        return None if (not math.isfinite(f)) else f  # filtra NaN e inf/-inf
    except Exception:
        return None
...
def build_dim_date(pool):
    ...
    sql = """
        INSERT IGNORE INTO dim_date
            (date_sk, year, month, month_name, quarter, semester,
             is_leap_year, is_satellite_era, is_omi_era)
    ...
    for year in range(YEAR_START, YEAR_END + 1):
        for month in range(1, 13):
            ...
            rows.append((
                ...
                month_names[month - 1],
                (month - 1) // 3 + 1,          # quarter
                1 if month <= 6 else 2,         # semester
                is_leap,
                year >= 2004,                   # is_satellite_era (Landsat 2004+)
                year < 2018,                    # is_omi_era (OMI NO2 pre-S5P)
            ))
