...
def export_spark_mode(pg: dict, tables: list, dry_run: bool) -> dict:
    ...
    for table in tables:
        ...
        try:
            ...
            err_str = str(e)
            if "UNABLE_TO_INFER_SCHEMA" in err_str or "PATH_NOT_FOUND" in err_str:
                print(f"    [SKIP] {table} no disponible en HDFS (tabla vacía o sin datos ETL)")
                results[table] = (True, 0)
            else:
                print(f"    [ERROR] {e}")
