...
from hashlib import md5  # Para firmar la solicitud (deduplicado)
...
# Mapea nombres amigables (ej: "tree_cover") a las URLs
...
def download_layer(service, layer, bbox, size=1024, crs="EPSG:3857"):
    ...
    Aplica deduplicado: si ya se descargó exactamente la misma petición,
    no vuelve a hacer la request.
    """
    ...
    if "geoserver" in url:
        params = {
            ...
            "srs": crs,  # Sistema de Coordenadas (EPSG:3857=Web Mercator)
            "styles": "",
            "format": "image/png",
            "transparent": "true"
        }
    ...
    # 🔹 DEDUPLICADO: calculamos firma de la solicitud
    ...
    # Guardamos la firma asociada a este PNG
...
if __name__ == "__main__":
    ...
    # 3. Redefinición de rutas (opcional, Docker-friendly)
    # Si quieres que afecte realmente a las globals, usa global:
    BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    DATA_DIR = os.path.join(BASE_DIR, "data", "hrl")
    LOG_DIR = os.path.join(BASE_DIR, "logs")
    ...
    try:
        ...
        # Captura cualquier error (ej: fallo de red, validación)
        log(f"❌ Error: {e}")
