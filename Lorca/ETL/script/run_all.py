import subprocess  # Permite ejecutar comandos de terminal desde Python (crea subprocesos)
...
# ======================================================
# CONFIG PATHS (Robusto por ubicación del archivo)
# ======================================================

# Directorio donde está ESTE archivo run_all.py
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Directorio donde viven tus scripts originales: ETL/script/*.py
ETL_SCRIPTS_DIR = os.path.join(BASE_DIR, "ETL", "script")

# ======================================================
# BLOQUE 1: Configuración del Pipeline (Flujo de Trabajo)
# ======================================================
...
SCRIPTS_GLOBALES = [
    ...
    {"file": "eurostat_extract.py",
     "args": ["--datasets", "all", "--eager-merge"]},

    {"file": "OECD_extract.py",
     "args": ["--pause", "300", "--penalty-429", "300,600,1200"]},
...
# ✅ NUEVO: scripts finales fuera de ETL/script
SCRIPTS_FINALES = [
    # merge.py está en la misma carpeta que este run_all.py
    {"path": os.path.join(BASE_DIR, "merge.py"), "args": []},

    # spark.py está 2 carpetas atrás y dentro de BBDD/
    {"path": os.path.join(BASE_DIR, "..", "..", "BBDD", "spark.py"), "args": []},
]
...
def generar_tareas_ciudad(nombre_ciudad, lat, lon):
    ...
    # Lista de tareas específicas para esta ciudad (6 tareas)
    return [
        # --- 1-2. HRL (Capas Estáticas) ---
        {
            "file": "HRL_extract.py",
        ...
        # --- 3-5. SENTINEL-2 (Imágenes Visuales y Datos) ---
        {
        ...
        # --- 6. SENTINEL-5P (Contaminación NO2) ---
        {
...
def ejecutar_script(script):
    ...
    Soporta:
      - {"file": "..."} -> busca dentro de ETL/script/
      - {"path": "..."} -> ruta directa (merge.py y spark.py)
    """
    ...
    print(f"\n▶️ Ejecutando {file_label} {' '.join(args)}")
    if env_vars:
    ...
    if process.returncode == 0:
        print(f"✅ {file_label} completado correctamente.\n")
        return True
    else:
        print(f"❌ Error al ejecutar {file_label} (código {process.returncode}).\n")
        return False
...
def main():
    ...
    completados = 0
    total_tareas = len(SCRIPTS_GLOBALES) + (num_ciudades * 6) + len(SCRIPTS_FINALES)
    ...
    for nombre_ciudad, (lat, lon) in EURO_FUAS.items():
        print(f"\n---------------------------------------------------")
    ...
    # 3) ✅ POST-PROCESO FINAL: merge.py y spark.py (spark SIEMPRE el último)
    print("\n🧩 FASE 3: POST-PROCESO (MERGE + SPARK)")
    for script in SCRIPTS_FINALES:
        if ejecutar_script(script):
            completados += 1
    ...
    print("==================================================================")
    print(f"\n🏁 ETL Finalizado: {completados} tareas completadas de {total_tareas}.")
