...
Fuente: EDGAR v8.0 FT2022 — Fossil CO2 emissions by country
  URL: https://jeodpp.jrc.ec.europa.eu/ftp/jrc-opendata/EDGAR/datasets/v80_FT2022/GHG/
  Dataset: IEA_EDGAR_CO2_1970_2022
...
# CONFIGURACIÓN
# ==============================================================================
# EDGAR v8.0 FT2022 — Fossil CO2 country totals, 1970-2022
EDGAR_URL     = (
    "https://jeodpp.jrc.ec.europa.eu/ftp/jrc-opendata/EDGAR/datasets/"
    "v80_FT2022/GHG/IEA_EDGAR_CO2_1970_2022.zip"
)
...
def parse_edgar_csv() -> pd.DataFrame:
    ...
    with zipfile.ZipFile(CACHE_ZIP) as zf:
        ...
        if not csv_names:
            raise RuntimeError(f"ZIP de EDGAR no contiene archivos .csv: {list(zf.namelist())[:10]}")
    ...
    print(f"  [OK] {len(df_raw):,} filas leídas | Columnas: {list(df_raw.columns[:8])}")
    ...
    if country_col is None:
        # Heurística: primera columna con valores de 3 letras mayúsculas
    ...
    if country_col is None:
        raise RuntimeError(
            f"No se detectó columna de código ISO3 en el CSV de EDGAR.\n"
    ...
    year_cols = {}
    for col in df_raw.columns:
        col_clean = col.strip().lstrip("Y_").lstrip("y_")
        try:
            yr = int(col_clean)
            if 1970 <= yr <= 2030:
                year_cols[col] = yr
    ...
    if not year_cols:
        raise RuntimeError(
            f"No se detectaron columnas de año en el CSV de EDGAR.\n"
    ...
    # Agregar por país (suma sectores si hay desglose sectorial)
    records = []
...
def main():
    ...
    print("=" * 60)
    print("  GTP — EXTRACCIÓN CO2 (EDGAR v8 FT2022 — JRC/EU)")
    print(f"  Fuente:  EDGAR v8.0 FT2022 (IEA_EDGAR_CO2_1970_2022)")
