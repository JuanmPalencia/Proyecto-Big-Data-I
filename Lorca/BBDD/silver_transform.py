...
def build_fact_economic(spark, pool, city_sk_map: dict):
    ...
    if ren is not None:
        ...
        # Asegurar month presente; si no hay, crear month=None para join
        ...
        for idx, year in enumerate(years):
            ...
            for month in range(1, 13):
                ...
                if tur_pdf is not None:
                    # prefer city-level; fallback country-level
                    if tur_pdf["month"].isna().all():
                        ...
                        # try city-level first if column exists
