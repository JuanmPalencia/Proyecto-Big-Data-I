# %%
import os, sys, re, io, subprocess, importlib.util
from pathlib import Path

# ============================================================
# 0) ✅ AJUSTES PRINCIPALES (EDITA SOLO ESTO SI CAMBIAN RUTAS)
# ============================================================
...
KUZNETS_DIR       = Path(r"/home/223B3336juan/Big Data I/data/DatosProcesados")
...
print("socketserver:", socketserver.__file__)
print("UnixStreamServer exists?", hasattr(socketserver, "UnixStreamServer"))
...
# Esperados en config.py (usa los que tengas):
# - EURO_FUAS: dict { "Madrid_ES": (lat, lon), ... }
# - CITY_MAPPING: dict { "Madrid_ES": ("ES001","ES300"), ... }  (metro_code, nuts3_code)
# - (opcional) FINANCE_PROXY_MAP: dict { "BE": ["FR","NL"], ... }  # fallback fin por país
# - (opcional) NO_NORMALIZE: set([...])  # eurostat datasets a no normalizar
...
# ============================================================
# 4) RESOLVER PROJECT_ROOT (desde PROJECT_ROOT_HINT, no desde cwd)
# ============================================================
...
CRUDO       = DATA_ROOT / "raw"
PROCESADO   = DATA_ROOT / "processed"
MODEL_READY = DATA_ROOT / "model_ready"

for p in [CRUDO, PROCESADO, MODEL_READY]:
...
def write_parquet(df, path: Path, mode="overwrite"):
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        df.write.mode(mode).format("parquet").save(spark_path(path))
        print(f"   ✔ Parquet escrito en {path}")
    except Exception as e:
        log_error(f"write_parquet({path})", e)
...
def safe_read_parquet(path: Path, context: str):
    try:
        if not path.exists():
            raise FileNotFoundError(f"No existe el parquet: {path}")
        return spark.read.parquet(spark_path(path))
    except Exception as e:
        log_error(f"lectura PARQUET ({context})", e)
        return None
...
write_parquet(fuas_df, PROCESADO / "geo" / "fuas")
print("✅ GEO FUA generado.")

# ============================================================
# 7) EUROSTAT — FASE 1/2 + PANEL
# ============================================================
EUROSTAT_IN  = CRUDO / "eurostat"
EUROSTAT_OUT = PROCESADO / "eurostat"
euro_logs = []
...
for csv_path in csv_files:
    ...
    write_parquet(df, EUROSTAT_OUT / name)
...
if euro_logs:
    ...
    write_parquet(euro_log_df, EUROSTAT_OUT / "_logs_phase12")
    print("✅ euro_logs guardados.")
else:
    print(f"⚠ No se generó ningún log de Eurostat. Revisa CSVs en: {EUROSTAT_IN}")

# ============================================================
# 7B) EUROSTAT — GDP per capita por ciudad (Metro -> NUTS3 -> national avg)
#     (CITY_MAPPING viene de config.py)
# ============================================================
EUROSTAT_CITY_OUT = PROCESADO / "eurostat" / "gdp_city_per_capita"
...
if (not METRO_TSV_GZ.exists()) or (not NUTS3_TSV_GZ.exists()):
    ...
    print("   Esperados:", METRO_TSV_GZ.name, "y", NUTS3_TSV_GZ.name)
else:
    ...
    if metro_long is None or nuts3_long is None:
        ...
        if len(CITY_MAPPING) == 0:
            ...
            fuas = safe_read_parquet(PROCESADO / "geo" / "fuas", "geo fuas")
            if fuas is not None:
                combo = combo.join(fuas.select(F.col("fua_id").alias("city_id2"), "lat", "lon"), combo.city_id == F.col("city_id2"), "left") \
                             .drop("city_id2")

            write_parquet(combo.dropDuplicates(["city_id", "year"]), EUROSTAT_CITY_OUT)
            print("✅ GDP city per capita generado en:", EUROSTAT_CITY_OUT.as_posix())
...
def euro_panelize(varname: str, unit: str | None = None):
    path_parquet = EUROSTAT_OUT / varname
    print(f"\n📂 Panelizando {varname} desde {path_parquet}")
    df = safe_read_parquet(path_parquet, f"eurostat {varname}")
    if df is None:
        return None
...
if panel is not None:
    EUROSTAT_PANEL_OUT = MODEL_READY / "eurostat_panel"
    panel.coalesce(1).write.mode("overwrite").parquet(spark_path(EUROSTAT_PANEL_OUT))
    print("✅ eurostat_panel guardado en:", EUROSTAT_PANEL_OUT.as_posix())
else:
...
if not OECD_IN.exists():
    ...
    for csv_path in list_csv_files(OECD_IN):
        ...
        write_parquet(df3, OECD_OUT / var)
        oecd_meta.append((var, was_wide))
        print(f"   ✔ Guardado parquet {var} | wide={was_wide}")

    if oecd_meta:
        oecd_meta_df = spark.createDataFrame(oecd_meta, ["dataset", "was_wide"])
        write_parquet(oecd_meta_df, OECD_OUT / "_logs_phase12")
        print("✅ oecd_logs guardados.")
    else:
        print(f"⚠ No se generó log OECD. Revisa CSVs en: {OECD_IN}")

# ============================================================
# 9) InvestEU — FASE 1/2/3 (country-level)
# ============================================================
INV_IN  = CRUDO / "investeu"
INV_OUT = PROCESADO / "investeu"
...
if ops is None or finals is None:
    ...
    amount_candidates = [
        ...
        if "amount" in c.lower() and ("eur" in c.lower() or "euro" in c.lower())
    ]
    ...
    INV_COUNTRY_OUT = MODEL_READY / "investeu_country"
    inv.coalesce(1).write.mode("overwrite").parquet(spark_path(INV_COUNTRY_OUT))
    print("✅ InvestEU guardado en:", INV_COUNTRY_OUT.as_posix())

# ============================================================
# 10) INE PIB — series trimestrales
# ============================================================
...
INE_OUT = MODEL_READY / "ine_timeseries"
ine_f3.coalesce(1).write.mode("overwrite").parquet(spark_path(INE_OUT))
print("✅ INE timeseries guardado en:", INE_OUT.as_posix())

# ============================================================
# 11) KUZNETS (CSV EXTERNO) -> parquet (model_ready/kuznets)
# ============================================================
...
KUZ_OUT = MODEL_READY / "kuznets"
kuz.coalesce(1).write.mode("overwrite").parquet(spark_path(KUZ_OUT))
print("✅ kuznets guardado en:", KUZ_OUT.as_posix())

# ============================================================
# 12) YFINANCE (nuevo) — Panel anual (Ticker-Year)
# ============================================================
FIN_RAW_NEW = PROJECT_ROOT / "data" / "raw" / "finance" / "financial_market_data.csv"
FIN_PANEL_OUT = PROJECT_ROOT / "data" / "model_ready" / "finance_panel"
FIN_CTRY_OUT  = PROJECT_ROOT / "data" / "model_ready" / "finance_country_year"
...
fin = (
    fin.withColumn("ticker", F.upper(F.trim(F.col("ticker").cast("string"))))
       ...
       .withColumn("country", F.upper(F.trim(F.col("country").cast("string"))))  # nombre (SPAIN, FRANCE...) o código
...
FIN_PANEL_OUT.mkdir(parents=True, exist_ok=True)
fin_panel.coalesce(1).write.mode("overwrite").parquet(spark_path(FIN_PANEL_OUT))
print(f"✅ finance_panel listo en: {FIN_PANEL_OUT.as_posix()}")
...
FIN_CTRY_OUT.mkdir(parents=True, exist_ok=True)
fin_country_year.coalesce(1).write.mode("overwrite").parquet(spark_path(FIN_CTRY_OUT))
print(f"✅ finance_country_year listo en: {FIN_CTRY_OUT.as_posix()}")

# ============================================================
# 13) MASTER DATASET — Limpieza + tipado + arrays + IMPUTACIONES 
#   cambios realizados:
#   A) NO2_* faltantes -> media de esa ciudad para ese año
#   B) Imperviousness faltante -> valor 2018 de esa ciudad
#   C) Países sin fin -> proxy países (ej BE = mean(FR, NL)) usando FINANCE_PROXY_MAP
# ============================================================
MASTER_IN  = PROJECT_ROOT / "data" / "Master_Dataset_Final.csv"
MASTER_OUT = MODEL_READY / "master_dataset"
...
if "source" in md.columns:
    md = md.withColumn("source", F.trim(F.col("source").cast("string")))

# --- Parseo listas a array<string> ---
...
# ============================================================
# 13A) IMPUTACIÓN NO2_* por media ciudad-año
# ============================================================
...
if len(NO2_COLS) > 0 and ("fua_id" in md.columns) and ("year" in md.columns):
    ...
    for c in NO2_COLS:
        md = md.withColumn(c, F.coalesce(F.col(c), F.col(f"{c}__cy_mean")))
        md = md.drop(f"{c}__cy_mean")
    print("✅ NO2 imputado por media ciudad-año.")
else:
    print("ℹ NO2 imputación saltada (faltan columnas o keys).")

# ============================================================
# 13B) IMPUTACIÓN Imperviousness por valor 2018 de esa ciudad
# ============================================================
...
if len(IMP_COLS) > 0 and ("fua_id" in md.columns) and ("year" in md.columns):
    ...
    for c in IMP_COLS:
        md = md.withColumn(c, F.coalesce(F.col(c), F.col(f"{c}__2018")))
        md = md.drop(f"{c}__2018")
    print("✅ Imperviousness imputado usando valor 2018 por ciudad.")
else:
    print("ℹ Imperviousness imputación saltada (faltan columnas o keys).")

# ============================================================
# 13C) IMPUTACIÓN FIN por país usando proxies (FINANCE_PROXY_MAP)
#     - Si geo_std no tiene fin1/fin2, usamos la media de países proxy.
#     - Ej BE: mean(FR, NL)
# ============================================================
FIN_COLS = [c for c in [
    "fin1_sector_avg_price","fin1_sector_avg_volatility",
...
if len(FIN_COLS) > 0 and ("geo_std" in md.columns) and ("year" in md.columns):
    # 1) tabla base country-year con medias de FIN desde el propio master (si ya existían algunos)
    fin_base = (
        md.groupBy("geo_std","year")
          .agg(*[F.avg(c).alias(f"{c}__base") for c in FIN_COLS])
    )

    # 2) construir tabla proxy (country-year) => media de los base de sus proxies
    proxy_rows = []
    ...
    fin_proxy = (
        proxy_map_df.join(fin_base.select(F.col("geo_std").alias("proxy_geo_std"), "year", *[F.col(f"{c}__base") for c in FIN_COLS]),
                          on=["proxy_geo_std"], how="left")
                    .groupBy("geo_std", "year")
                    .agg(*[F.avg(F.col(f"{c}__base")).alias(f"{c}__proxy") for c in FIN_COLS])
    )

    # 3) aplicar imputación en md: si FIN null, usar proxy
    md = md.join(fin_proxy, on=["geo_std","year"], how="left")
    for c in FIN_COLS:
        md = md.withColumn(c, F.coalesce(F.col(c), F.col(f"{c}__proxy")))
        md = md.drop(f"{c}__proxy")

    print("✅ FIN imputado por proxies de país (FINANCE_PROXY_MAP).")
else:
    print("ℹ FIN proxy imputación saltada (faltan columnas o keys).")

# ============================================================
# 13D) Guardar master_dataset parquet
# ============================================================
md.coalesce(1).write.mode("overwrite").parquet(spark_path(MASTER_OUT))
print(f"✅ master_dataset guardado en: {MASTER_OUT.as_posix()}")

prev_md = spark.read.parquet(spark_path(MASTER_OUT))
print("➡️ master_dataset | filas:", prev_md.count(), "| cols:", len(prev_md.columns))
prev_md.show(10, truncate=False)

# ============================================================
# 14) CHECKS FINALES + RESÚMENES DE CALIDAD
# ============================================================
print("\n====================")
print("✅ CHECKS FINALES")
print("====================")

md_final = safe_read_parquet(MASTER_OUT, "master_dataset final")
if md_final is None:
    raise RuntimeError("❌ No pude leer el parquet final del master_dataset.")

# --- columnas clave esperadas ---
keys = [c for c in ["fua_id", "geo_std", "year"] if c in md_final.columns]
print("Keys detectadas:", keys)

# --- duplicados por key (si existen las 3) ---
if set(["fua_id","year"]).issubset(set(md_final.columns)):
    dup = (
        md_final.groupBy("fua_id","year")
                .count()
                .filter(F.col("count") > 1)
    )
    dup_count = dup.count()
    print("Duplicados (fua_id,year) > 1:", dup_count)
    if dup_count > 0:
        dup.show(20, truncate=False)
else:
    print("ℹ No puedo chequear duplicados por (fua_id,year) porque faltan columnas.")

# --- nulos en variables importantes ---
important = [c for c in [
    "no2_mean","no2_max","no2_min","no2_std",
    "imperviousness_mean","imperviousness_pct_cover","imperviousness_pct_dense",
    "gdp_per_capita",
    "fin1_sector_avg_price","fin1_sector_avg_volatility",
    "fin2_sector_avg_pe","fin2_sector_avg_beta","fin2_sector_avg_return","fin2_sector_avg_volatility"
] if c in md_final.columns]

if important:
    null_stats = md_final.select([
        F.sum(F.col(c).isNull().cast("int")).alias(f"{c}__nulls")
        for c in important
    ])
    print("\n📌 Nulos por columna (important):")
    null_stats.show(truncate=False)
else:
    print("ℹ No hay columnas 'important' presentes para medir nulos.")

# --- rangos básicos (si existen) ---
range_checks = []

def add_range_check(colname, low=None, high=None):
    if colname in md_final.columns:
        cond = F.lit(False)
        if low is not None:
            cond = cond | (F.col(colname) < F.lit(low))
        if high is not None:
            cond = cond | (F.col(colname) > F.lit(high))
        range_checks.append(
            F.sum(cond.cast("int")).alias(f"{colname}__out_of_range")
        )

add_range_check("ndvi_mean", -1, 1)
add_range_check("pct_green_area", 0, 100)
add_range_check("pct_grey_area", 0, 100)
add_range_check("imperviousness_pct_cover", 0, 100)
add_range_check("imperviousness_pct_dense", 0, 100)

if range_checks:
    print("\n📌 Out-of-range counts:")
    md_final.select(range_checks).show(truncate=False)

# ============================================================
# 15) RESUMEN DE IMPUTACIONES (CUÁNTO SE RELLENÓ)
#     Calcula aproximado: compara nulos antes/después si guardaste md previo.
#     Como ya escribimos MASTER_OUT, hacemos resumen post.
# ============================================================
print("\n====================")
...
if ("fua_id" in md_final.columns) and ("year" in md_final.columns):
    # % de NO2 completo por ciudad-año
    no2_cols = [c for c in ["no2_mean","no2_max","no2_min","no2_std"] if c in md_final.columns]
    if no2_cols:
        no2_completeness = (
            md_final.groupBy("fua_id","year")
                    .agg(*[F.avg(F.col(c).isNotNull().cast("int")).alias(f"{c}__notnull_rate") for c in no2_cols])
        )
        print("\n✅ NO2 completeness (rate not null por city-year) — muestra:")
        no2_completeness.show(10, truncate=False)

    imp_cols = [c for c in ["imperviousness_mean","imperviousness_pct_cover","imperviousness_pct_dense"] if c in md_final.columns]
    if imp_cols:
        imp_nulls = md_final.select([F.sum(F.col(c).isNull().cast("int")).alias(f"{c}__nulls") for c in imp_cols])
        print("\n✅ Imperviousness nulls (post):")
        imp_nulls.show(truncate=False)
...
if fin_cols:
    fin_nulls = md_final.select([F.sum(F.col(c).isNull().cast("int")).alias(f"{c}__nulls") for c in fin_cols])
    print("\n✅ Finance nulls (post):")
    fin_nulls.show(truncate=False)

# ============================================================
# 16) EXPORT OPCIONAL A CSV ÚNICO (si lo necesitas)
# ============================================================
EXPORT_CSV = False  # pon True si lo quieres
MASTER_CSV_OUT = MODEL_READY / "master_dataset_csv"

if EXPORT_CSV:
    print("\n📤 Exportando master_dataset a CSV (1 partición) ...")
    (
        md_final.coalesce(1)
               .write.mode("overwrite")
               .option("header", True)
               .csv(spark_path(MASTER_CSV_OUT))
    )
    print("✅ CSV exportado en:", MASTER_CSV_OUT.as_posix())
else:
    print("\nℹ Export CSV desactivado (EXPORT_CSV=False).")

# ============================================================
# 17) STOP SPARK (limpio)
# ============================================================
print("\n🧹 Parando Spark...")
spark.stop()
